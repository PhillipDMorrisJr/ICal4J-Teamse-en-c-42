...
public class CalendarValidatorImpl implements Validator<Calendar> {
	...
	public void validate(Calendar target) throws ValidationException {
			...
			try {
				if (!Version.VERSION_2_0.equals(target.getProperty(Property.VERSION))) {
					throw new ValidationException(
							"Unsupported Version: " + target.getProperty(Property.VERSION).getValue());
				}
		...
		// but MUST NOT occur more than once
		PropertyValidator.getInstance().assertOneOrLess(Property.CALSCALE, target.getProperties());
		PropertyValidator.getInstance().assertOneOrLess(Property.METHOD, target.getProperties());
		...
		for (final Property property : target.getProperties()) {
			boolean isCalendarProperty = CollectionUtils.find(calendarProperties,
					new Predicate<Class<? extends Property>>() {
						@Override
		...
		Method method;
		try {
			method = (Method) target.getProperty(Property.METHOD);
			...
			if (Method.PUBLISH.equals(method)) {
				if (target.getComponent(Component.VEVENT) != null) {
					...
					// ComponentValidator.assertNone(Component.VFREEBUSY,
					// target.getComponents());
					// ComponentValidator.assertNone(Component.VEVENT,
					// target.getComponents());
					ComponentValidator.assertNone(Component.VJOURNAL, target.getComponents());
				}
				...
				// ComponentValidator.assertNone(Component.VEVENT,
				// target.getComponents());
				// ComponentValidator.assertNone(Component.VTODO,
				// target.getComponents());
				// }
